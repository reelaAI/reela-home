  ...
  return (
    <section class='fade-in-up container mx-auto px-4 py-20 text-center'>
      <div class='mx-auto flex max-w-6xl flex-col items-center text-center'>
        <h1 class='font-inter font-bold leading-tight tracking-tight text-[4.5rem] text-black'>
          Video Creation, <span class='text-brand'>Automated.</span>
        </h1>
        ...
        <div class='mt-12 grid w-full max-w-5xl gap-4 text-left md:grid-cols-3'>
          {heroHighlights.map((highlight) => (
            <div
              key={highlight.title}
              class='flex h-full items-center gap-4 rounded-3xl border border-gray-200 bg-white/95 p-4 shadow-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-md md:p-5'
            >
              <div class='flex h-12 w-12 items-center justify-center rounded-2xl bg-brand/10'>
                <Icon
                  name={highlight.icon}
        ...
        <div class='mt-10 flex flex-col items-center gap-6'>
          <a
            class='inline-flex items-center justify-center rounded-full bg-brand px-8 py-4 text-base font-semibold text-white shadow-sm transition-all duration-200 hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2'
                ...
                {[brands, brands, brands].flat().map((brand, index) => (
                  ...
                  >
                    <img
                      ...
                      loading='lazy'
                      src={brand.src}
                      style='color: transparent; filter: brightness(0) saturate(100%) invert(32%) sepia(3%) saturate(0%) hue-rotate(213deg) brightness(98%) contrast(89%);'
                      width='80'
                    />
