...
const Section1 = () => {
  ...
  return (
    <section class='fade-in-up container mx-auto px-4 py-20 text-center'>
      <div class='mx-auto flex max-w-6xl flex-col items-center text-center text-gray-900'>
        <h1 class='font-inter font-bold leading-tight tracking-tight text-[4.5rem]'>
          Video Creation,
          <span class='ml-4 inline-flex items-center rounded-full bg-black px-6 py-2 text-[0.7em] font-semibold uppercase tracking-[0.2em] text-white shadow-sm md:ml-6'>
            Automated
          </span>
        </h1>
        <p class='font-inter mt-6 max-w-4xl text-base text-gray-700 md:text-lg'>
        ...
        <div class='mt-12 grid w-full max-w-5xl gap-4 text-left md:grid-cols-3'>
          {heroHighlights.map((highlight) => (
            <div
              key={highlight.title}
              class='flex h-full items-center gap-4 rounded-3xl border border-gray-200/70 bg-white/95 p-4 shadow-sm backdrop-blur-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-lg md:p-5'
            >
              <div class='flex h-12 w-12 items-center justify-center rounded-2xl bg-black/5'>
                <Icon
                  name={highlight.icon}
        ...
        <div class='mt-10 flex flex-col items-center gap-6'>
          <a
            class='inline-flex items-center justify-center rounded-full bg-black px-8 py-4 text-base font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2'
        ...
        <div class='mt-14 flex w-full items-center justify-center px-4'>
          <div class='flex flex-wrap items-center justify-center gap-6'>
            <p class='shrink-0 whitespace-nowrap font-medium text-gray-900'>Powered by industry-leading AI models:</p>
            ...
            <div class='relative max-w-[90vw] overflow-x-hidden md:max-w-2xl'>
              <div class='animate-infinite-scroll mt-10 flex w-[1000%] md:mt-0'>
                {[brands, brands, brands].flat().map((brand, index) => (
                  <div
                    class='mx-8 flex flex-shrink-0 flex-col items-center justify-center gap-2 text-gray-500'
                    key={`${brand.name}-${index}`}
                  >
                    <img
                      alt={`${brand.name} logo`}
                      class='h-8 w-20 object-contain opacity-80'
                      decoding='async'
                      height='32'
                      loading='lazy'
                      src={brand.src}
                      style='color: transparent; filter: grayscale(100%) contrast(110%);'
                      width='80'
                    />
