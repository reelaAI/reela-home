    ...
    <section class='fade-in-up container mx-auto px-4 py-20 text-center'>
      <div class='mx-auto flex max-w-6xl flex-col items-center text-center'>
        <h1 class='font-inter font-bold leading-tight tracking-tight text-[4.5rem] text-gray-900'>
          Video Creation, <span class='text-[#ff0058]'>Automated.</span>
        </h1>
        ...
        <div class='mt-12 grid w-full max-w-5xl gap-4 text-left md:grid-cols-3'>
          {heroHighlights.map((highlight) => (
            <div
              key={highlight.title}
              class='flex h-full items-center gap-4 rounded-3xl border border-gray-100 bg-white/90 p-4 shadow-sm backdrop-blur-sm transition-transform duration-200 hover:-translate-y-1 hover:shadow-lg md:p-5'
            >
              <div class='flex h-12 w-12 items-center justify-center rounded-2xl bg-[#ff0058]/10'>
                <Icon
                  name={highlight.icon}
                  className='h-8 w-8 text-[#ff0058]'
                />
              </div>
        ...
        <div class='mt-10 flex flex-col items-center gap-6'>
          <a
            class='inline-flex items-center justify-center rounded-full bg-[#ff0058] px-8 py-4 text-base font-semibold text-white shadow-sm transition-all duration-200 hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ff0058] focus-visible:ring-offset-2'
                ...
                {[brands, brands, brands].flat().map((brand, index) => (
                  ...
                  >
                    <img
                      ...
                      loading='lazy'
                      src={brand.src}
                      style='color: transparent; filter: grayscale(1); opacity: 0.75;'
                      width='80'
                    />
