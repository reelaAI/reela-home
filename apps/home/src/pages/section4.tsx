    ...
    <Fragment>
      <div class='container mx-auto mb-12 px-4 py-16 text-center'>
        <h2 class='font-inter text-[3rem] font-bold text-white'>
          ...
          Video Creation
          <p class='mt-6 mx-auto max-w-3xl font-inter text-base font-semibold text-slate-300 md:text-lg'>
      ...
      <section class='container mx-auto mb-8 px-4'>
        <div class='mx-auto max-w-[1200px]'>
          <div class='mb-8 grid grid-cols-1 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] shadow-[0_30px_120px_rgba(0,0,0,0.45)] backdrop-blur-xl p-6 md:p-12 lg:grid-cols-[minmax(100px,250px)_1fr]'>
            {config.map((item, index) => (
              <div key={String(index)} class='contents group'>
                <label class='label-sm col-start-1 max-h-max w-full overflow-hidden rounded-lg border border-white/10 bg-white/5 p-4 transition-all duration-200 hover:border-brand/60 hover:bg-white/10'>
                ...
                <div class='relative row-start-6 hidden h-full w-full rounded-xl border border-white/10 bg-black/80 shadow-[0_20px_100px_rgba(0,0,0,0.6)] lg:col-start-2 lg:row-start-1 group-has-checked:block'>
          ...
          <div class='flex flex-wrap items-center justify-center gap-2 text-center text-white'>
            ...
            <a href='/workspace'>
              <button class='inline-flex items-center rounded-full bg-brand px-6 py-3 font-medium text-white shadow-[0_20px_60px_rgba(255,0,88,0.35)] transition-transform hover:scale-105'>
                are waiting for you
