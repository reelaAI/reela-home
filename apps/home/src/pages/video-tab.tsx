        ...
        <div class='grid gap-8 md:grid-cols-2'>
          <div class='flex flex-col'>
            <div class='space-y-6'>
              {videoWithId.map((video, index) => (
                <label
                  key={video.id}
                  class={`${tabName} ${index === 0 ? 'border-gray-900 bg-white border-l-4' : 'bg-white/50'} block w-full rounded-xl p-5 text-left shadow-md transition-all duration-200 hover:bg-white hover:shadow-sm`}
                  for={video.id}
                  onclick={`videoTabClick(this, '${tabName}')`}
                >
                  <h3
                    class={
                      'mb-2 text-xl font-bold text-gray-800 ' + (index === 0 ? 'text-gray-900' : '')
                    }
                  >
                    {video.title}
                  </h3>
          ...
          <div class='flex flex-col'>
            ...
            <div class='mt-6 flex justify-end'>
              <a href='/workspace'>
                <a
                  class='bg-gray-900 rounded-full px-8 py-3 font-medium text-white transition-transform hover:scale-110'
                  href={tabButtonSrc}
                >
                  {tabButton}
                </a>
