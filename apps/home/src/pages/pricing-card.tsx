...
const PricingCard: FC<Props> = ({
  ...
  return (
    ...
    >
      {/* chips */}
      {isPopular && (
        <div className='absolute left-0 top-0 rounded-br-xl bg-neutral-950 px-2 py-1 text-xs text-white'>
          Popular
        </div>
      )}
      ...
      <a
        ...
        className={cn(
          ...
          {
            'bg-black': !isPopular,
            'bg-neutral-900 text-white': isPopular,
          },
        ...
        {features.map((feature, index) => {
          ...
          return (
            <div key={index} className='my-2 flex items-center gap-2'>
              <Icon name='check' className='h-4 w-4 shrink-0 text-black' />
