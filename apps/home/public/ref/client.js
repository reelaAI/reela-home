...
function activateLabel(element) {
  ...
  labels.forEach((label) => {
    label.classList.remove('text-[#0F4C81]');
  });
  element.classList.remove('text-gray-700', 'hover:text-gray-900');
  element.classList.add('text-[#0F4C81]');
}

function videoTabClick(element, tabName) {
  const labels = document.querySelectorAll('.' + tabName);
  labels.forEach((label) => {
    label.classList.remove('bg-white', 'shadow-md', 'border-l-4', 'border-[#0F4C81]');
    label.classList.add('bg-white/50');
    label.querySelector('h3').classList.remove('text-[#0F4C81]');
  });

  element.classList.add('bg-white', 'shadow-md', 'border-l-4', 'border-[#0F4C81]');
  element.classList.remove('bg-white/50');
  element.querySelector('h3').classList.add('text-[#0F4C81]');
